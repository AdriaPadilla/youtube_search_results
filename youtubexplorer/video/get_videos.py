...
import youtubexplorer.video.video as vid
...
def get_videos(query, max_results):
	...
	for result in results:	
		...
		search_response_object = vid.Video(
			query,
...
def explore_videos(query, max_results, get_related, iteration_n):
	...
	if get_related == True:
		...
		related_videos_list = get_related_videos(seed_list, max_results)
		...
		while iteration_counter < iteration_n:
			...
			more_related_videos = get_related_videos(seed_list, max_results)
...
def explore_related(seed_list, max_related, iteration_n):
	...
	related_videos = get_related_videos(seed_list, max_related)
	...
	while iteration_counter < iteration_n:
		...
		more_related_videos = get_related_videos(seed_list, max_related)
		videos.extend(more_related_videos)
		iteration_counter += 1
